...
1. Отправляется заявка на займ. Выполняется http-метод [Request](#Request).
...
После ввода ему в СМС или на e-mail должен прийти пароль для входа. Для получения пароля и входа в личный кабинет используется http-метод [client](#avtorizatsiya-klienta-v-a-id-client-lk-a).
3. Сразу после авторизации выполняется http-метод [NumbersDeals](#NumbersDeals), который получает все номера заявок и договоров текущего клиента.
4. Далее может быть выполнен http-метод [StatusDeal](#StatusDeal), который получает статусы заявок.
...
6. Если клиент подал заявку и она одобрена, он может ознакомиться с договором, который может быть получен http-методом [dealinpdf](#dealinpdf).
7. На этом этапе или раньше клиент должен привязать банковскую карту. Привязанная карта передается в базу "Моя МФО" при помощи http-метода [card/attachtoclient](#card_attachtoclient) или card/attachtodeal.
8. Если клиент готов подписать договор вызывается http-метод [dealsign/code](#code), который отправляет сайту одноразовый код для подписания договора.
Данный код, должен быть указан клиентом как аналог собственоручной подписи.
9. Если клиент указал код, он должен быть отправлен сайтом на проверку в базу "Моя МФО" посредством http-метода [dealsign/codereview](#codereview). Если код указан верно возвращается подписанный договор.
...
В этом случае используется метод [Payment/Site](#payment_site).
...
## <a id="NumbersDeals">Получение всех номеров договоров (сделок) по одному клиенту</a>
...
## <a id="Request">Отправка заявки</a>
...
## <a id="statusdeal">Проверка статуса договора (сделки)</a>
...
## <a id="card_attachtoclient">Привязка банковской карты</a>
...
## <a id="dealinpdf">Получение договора в формате "pdf" (без подписи)</a>
...
## <a id="code">Запрос на получение кода АСП (аналога собственоручной подписи) из "Моя МФО"</a>
...
## <a id="codereview">Отправка указанного клиентом кода АСП и получение подписанного договора в формате "pdf"</a>
...
## <a id="ps_deal">Получение сводных данных о сумме текущей задолженности по конкретному договору для вывода в личном кабинете</a>
...
## <a id="payment_site">Отражение в системе "Моя МФО" полученного на сайте платежа (эквайринг)</a>
